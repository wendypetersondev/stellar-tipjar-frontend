...
import type { SelectedCreator } from "./TipPredictions";
...
export function CreatorSelector({ 
  ...
  const isCreatorSelected = (username: string) => {
    return selectedCreator?.username === username;
  };
    ...
    <div className="relative">
      <div className="relative">
        ...
        <input
          ...
          className="w-full pl-10 pr-4 py-3 border border-ink/20 rounded-xl bg-white/50 text-ink placeholder-ink/40 focus:outline-none focus:ring-2 focus:ring-wave/20 focus:border-wave"
        />
      </div>
              ...
              {searchResults.map((creator) => (
                ...
                >
                  ...
                  <Button
                    ...
                    disabled={isCreatorSelected(creator.username)}
                  >
                    {isCreatorSelected(creator.username) ? "Selected" : "Select"}
                  </Button>
                </div>
              ))}
            </div>
